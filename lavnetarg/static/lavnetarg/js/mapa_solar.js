...
const STATIONS = {
    transmitters: [
        { name: "NAA", lat: 50.00, lon: -48 },   
        { name: "NLK", lat: 51.00, lon: -110 },  
        { name: "NPM", lat: 21.42, lon: -145 },  
        { name: "NWC", lat: -26.00, lon: 130 },  
        { name: "NDK", lat: 52.00, lon: -85 },    
        { name: "NAU", lat: 20.00, lon: -46}   
    ],
    receivers: [
        { id: "ARG", name: "LAVNet-Ar", lat: -29.00, lon: -43, location: "Tucumán, AR" }
    ]
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Crear canvas fantasma para la sombra
    ...
    worldMapImage.onload = () => {
        ...
        // Aplicamos el filtro aquí (ajustado para que el blanco sea oscuro)
        ...
        // Reemplazamos la imagen original con el resultado filtrado
    ...
    worldMapImage.src = '/static/lavnetarg/images/mapamundi_pacifico.png';
...
function drawShadowLayer(sunPos) {
    ...
    // Dibujar la capa de sombra sobre el mapa principal
    ctx.drawImage(shadowCanvas, 0, 0, MAP_WIDTH, MAP_HEIGHT);
}

function latLonToXY(lat, lon) {
    // Offset de -11 para alinear el sol con la costa de este mapa específico
...
function drawTimeZoneLabel() {
    ...
    const zacReceiver = STATIONS.receivers.find(r => r.id === 'UAZ');
    const label = zacReceiver ? `UTC-6 (${zacReceiver.location})` : 'UTC-3 (Tucumán, Ar)';
