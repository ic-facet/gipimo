...
// Coordenadas de estaciones VLF (Actualizadas por el profesor)
const STATIONS = {
    transmitters: [
    ...
    receivers: [
        { id: "UNAM", name: "LAVNet-CDM-Mx", lat: 23.00, lon: -83, location: "Ciudad de México, MX" } // Alineado con tu punto de México
    ]
};

// Lista Extendida de Volcanes del Cinturón de Fuego
const VOLCANOES = [
    // --- MÉXICO (Sobre tu punto {lat: 23, lon: -92}) ---
    { name: "Popocatépetl", lat: 22.00, lon: -81 }, 
    { name: "Colima", lat: 22.00, lon: -86 },

    // --- CENTROAMÉRICA (Sobre tu punto {lat: 15, lon: -80}) ---
    { name: "Fuego", lat: 16.00, lon: -70 },

    // --- SUDAMÉRICA (Sobre tus puntos de lon: -60 a -55) ---
...
const TECTONIC_LINES = [
    ...
    // --- TRAMO DERECHO (América) - AJUSTE MILIMÉTRICO A LA COSTA ---
    [
        {lat: 67, lon: -140},
        {lat: 60, lon: -120}, // Alaska (más hacia la derecha para tocar tierra)
        {lat: 48, lon: -112}, // Cascadia / Canadá
        {lat: 34, lon: -105}, // California / USA
        {lat: 23, lon: -92},  // México (Entrando bien a la zona del Pacífico mexicano)
        {lat: 15, lon: -80}   // Centroamérica
    ],
    [
        {lat: 15, lon: -80}, 
        {lat: 5, lon: -58},   // Colombia
        {lat: -10, lon: -60}, // Perú (Empujado al borde de la silueta gris)
        {lat: -20, lon: -55},
        {lat: -24, lon: -49},
        {lat: -30, lon: -50}, // Chile Central
        {lat: -55, lon: -58}  // Tierra del Fuego
    ]
];
// Variables globales
let canvas, ctx;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    worldMapImage.src = '/static/lavnetunam/images/mapamundi_pacifico.png';
...
function updateMap() {
    ...
    drawTectonicPlates();
    drawVolcanoes();
    //drawPropagationLines(); // Líneas de conexión
    drawStations(); // Puntos de las estaciones
...
function drawShadowLayer(sunPos) {
    ...
    for (let y = 0; y < SHADOW_RES_Y; y++) {
        ...
        for (let x = 0; x < SHADOW_RES_X; x++) {
            ...
            // Color de la noche: Azul marino (R:10, G:20, B:60)
            data[i] = 10;     
            data[i + 1] = 20; 
            data[i + 2] = 60; 
            ...
            let alpha = 0;
            if (sinAlt > 0.05) {
                // DÍA PLENO: El sol está arriba, sin sombra.
                alpha = 0; 
            } else if (sinAlt < -0.1) {
                // NOCHE PLENA: Máxima oscuridad.
                alpha = 215; // 82% de opacidad para que no sea negro total
            } else {
                // CREPÚSCULO: Transición suave entre día y noche.
...
function drawPropagationLines() {
    // Buscar el receptor de Zacatecas (UAZ)
...
function drawTectonicPlates() {
    TECTONIC_LINES.forEach(path => {
        ...
        // 2. Línea Central Definida
        ctx.strokeStyle = '#ff4500';
        ctx.lineWidth = 2;
        ctx.setLineDash([5, 5]); // Línea punteada para estilo técnico
        ctx.beginPath();
        drawPath(path);
        ctx.stroke();
        ctx.setLineDash([]);
...
function drawStations() {
    ...
    STATIONS.transmitters.forEach(tx => {
        ...
        // EL TRUCO: Dibujar contorno primero
    ...
    STATIONS.receivers.forEach(rx => {
        ...
        // Contorno negro para el receptor
        ctx.strokeStyle = 'black';
        ctx.lineWidth = 3;
...
function drawTimeZoneLabel() {
    ...
    const zacReceiver = STATIONS.receivers.find(r => r.id === 'UAZ');
    const label = zacReceiver ? `UTC-6 (${zacReceiver.location})` : 'UTC-6 (CDMX, MX)';
